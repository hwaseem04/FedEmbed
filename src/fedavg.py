    ...
    def train(self, epochs=1): 
        ...
        for _ in range(epochs): 
            for img0, img1, flag, targets in self.dataloader:
                img0, _, _, target1, _ = img0.cuda(), img1.cuda(), flag.cuda(), targets[:, 0].long().cuda(), targets[:, 1].cuda()
                self.optimizer.zero_grad()

                output = self.model(img0)
...
clients = [Client(loader, i) for i, loader in enumerate(client_loaders[:-1])][:2]
...
for round in range(num_rounds):
    ...
    for idx, client in enumerate(clients):
        client_val_accuracy = classwise_accuracy(client.model, test_data_loader[-1])  # Use the client's model for evaluation
